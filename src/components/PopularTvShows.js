...
import { getPopularTvShows, discoverTvShows, getImageUrl } from "../api/tmdb";
...
const PopularTvShows = () => {
  ...
  useEffect(() => {
    if (!genreActive && previousStateRef.current !== null) {
      ...
      setPopularTvShows(snapshot.tvShows);
  ...
  }, [genreActive, popularTvShows, currentPage, totalPages, totalTvShows]);
  ...
  return (
    <div>
      ...
      {!showGenreView && (
        <>
          <div className="tvShows-container">
            {defaultLoading && popularTvShows.length === 0 ? (
              <div className="genre-loading">Loading...</div>
            ) : (
              popularTvShows.map((tvShow) => renderTvShowCard(tvShow))
            )}
          </div>
          <Pagination

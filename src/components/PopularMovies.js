...
import { getPopularMovies, discoverMovies, getImageUrl } from "../api/tmdb";
...
const PopularMovies = () => {
  ...
  useEffect(() => {
    if (!genreActive && previousStateRef.current !== null) {
      ...
      setPopularMovies(snapshot.movies);
  ...
  }, [genreActive, popularMovies, currentPage, totalPages, totalMovies]);
  ...
  return (
    <div>
      ...
      {!showGenreView && (
        <>
          <div className="movie-container">
            {defaultLoading && popularMovies.length === 0 ? (
              <div className="genre-loading">Loading...</div>
            ) : (
              popularMovies.map((movie) => renderMovieCard(movie))
            )}
          </div>
          <Pagination
